...
const char* fDataName = "tnp_Ana_Data_recoid.root";
const char* fMCName = "tnp_Ana_MC_recoid.root";
...
// Other parameters
TString etaTag("recoid_1bin");
TString ptTag("recoid_pt");
TString allTag("recoid_1bin");

// Automatic parameters (shouldn't require modification by the user)
const int nAbsEtaBins = 1;
TString cutTag("tpTree_recoid"); 
TString cutLegend("Photon reco+ID");
TString probetkEtaTag("recoid_1bin");
TString probetkEtaVar("probetkEta");
...
void TnPEffDraw() {
  ...
  cout << __LINE__ << endl;
  TGraphAsymmErrors *ComEta0 = plotEff_1bin(daEtaData0, 1, "probetkEta");
  cout << __LINE__ << endl;
  TGraphAsymmErrors *ComEta1 = plotEff_1bin(daEtaData1, 1, "probetkEta");
  cout << __LINE__ << endl;
  ...
  cout << __LINE__ << endl;
  RooDataSet *daAbsEtaMC1 = (RooDataSet*)f9->Get(cutTag + "/" + probetkEtaTag + "/fit_eff"); 
  cout << __LINE__ << endl;
  RooDataSet *daAbsEtaData1 = (RooDataSet*)f10->Get(cutTag + "/" + probetkEtaTag + "/fit_eff"); 
  cout << __LINE__ << endl;

  cout << __LINE__ << endl;
  TGraphAsymmErrors* Com0Pt0 = plotEff_1bin(daPtMC1Bin0,0,"probetkEta");
  cout << __LINE__ << endl;
  TGraphAsymmErrors* Com0Pt1 = plotEff_1bin(daPtData1Bin0,0,"probetkEta");
  cout << __LINE__ << endl;
  vector<TGraphAsymmErrors*> Com0AbsEta0 = plotEff_Nbins(daAbsEtaMC1,0,"probetkMinDpt",probetkEtaVar);
  cout << __LINE__ << endl;
  vector<TGraphAsymmErrors*> Com0AbsEta1 = plotEff_Nbins(daAbsEtaData1,0,"probetkMinDpt",probetkEtaVar);
  cout << __LINE__ << endl;


  int nbins_probetkEta = ComPt0.size();
  for (int i=0; i<nbins_probetkEta; i++)
  {
  ...
  double Trk0[4];
  double Trk1[4];
  double** TrkAbsEta0 = new double*[nbins_probetkEta];
  double** TrkAbsEta1 = new double*[nbins_probetkEta];
  for (int i=0; i<nbins_probetkEta; i++) 
  {
  ...
  for (int i=0; i<nbins_probetkEta; i++)
  {
     ...
     if (daPtData0[i]->get()->find("probetkEta"))
     {
        ...
        leg1->SetHeader(TString("#splitline{") + cutLegend + Form(" Efficiency}{(p_{T}^{e} - p_{T}^{tk}>%.1f, |#eta| #in [%.1f, %.1f])}",ptmin,etamin,etamax));
     }
     else
     {
        etamin = ((RooRealVar*) daPtData0[i]->get()->find("probetkEta"))->getBinning().binLow(0);
...
// #endif // ifdef MUIDTRG
  ...
  leg1->SetHeader(TString("#splitline{") + cutLegend + Form(" Efficiency}{(p_{T}^{e} - p_{T}^{tk}>%.1f)}",ptmin));
...
vector<TGraphAsymmErrors*> plotEff_Nbins(RooDataSet *a, int aa, const char* varx, const char* var2){
  const RooArgSet *set = a->get();
  RooRealVar *xAx = (RooRealVar*)set->find(varx);
  RooRealVar *probetkEta = (RooRealVar*)set->find(var2);
  RooRealVar *eff = (RooRealVar*)set->find("efficiency");

  const int nbins = xAx->getBinning().numBins();
  const int nbins2 = probetkEta->getBinning().numBins();
  ...
  for (int i=0; i<nbins*nbins2; i++) {
    ...
    int bin2 = probetkEta->getBinning().binNumber(probetkEta->getVal());
