void convEff() {
   ...
   // book histos
   TH1F *haxes_tk = new TH1F("haxes_tk",";photon E_{T} [GeV];no track prob.",1,0,20);
   TH1F *haxes_el = new TH1F("haxes_el",";photon E_{T} [GeV];electron finding prob.",1,0,20);
   TH1F *hnum_tk_data = new TH1F("hnum_tk_data",";photon E_{T} [GeV];no track prob.",nbins,bins);
   TH1F *hden_data = new TH1F("hden_data",";photon E_{T} [GeV];no track prob.",nbins,bins);
   TH1F *hnum_el_data = new TH1F("hnum_el_data",";photon E_{T} [GeV];electron finding prob.",nbins,bins);
   TH1F *hnum_tk_mc = new TH1F("hnum_tk_mc",";photon E_{T} [GeV];no track prob.",nbins,bins);
   TH1F *hden_mc = new TH1F("hden_mc",";photon E_{T} [GeV];no track prob.",nbins,bins);
   ...
   trdata->Project("hnum_tk_data","phoSCEt_notag","phoSCEt_notag>2&&probetkPt<2&&(tagCharge!=probetkCharge)&&!(nmatchtrk>0&&Min$(abs(gentrkdxy[imatchtrk])-abs(probetkdxy))>0)");
   trdata->Project("hnum_el_data","phoSCEt_notag","nmatchele>0&&phoSCEt_notag>2&&probetkPt<2&&(tagCharge!=probetkCharge)&&Min$(abs(eleD0[imatchele])-abs(probetkd0))>0");
   trdata->Project("hden_data","phoSCEt_notag","phoSCEt_notag>2&&probetkPt<2&&(tagCharge!=probetkCharge)");
   trmc->Project("hnum_tk_mc","phoSCEt_notag","phoSCEt_notag>2&&probetkPt<2&&(tagCharge!=probetkCharge)&&!(nmatchtrk>0&&Min$(abs(gentrkdxy[imatchtrk])-abs(probetkdxy))>0)");
   trmc->Project("hnum_el_mc","phoSCEt_notag","nmatchele>0&&phoSCEt_notag>2&&probetkPt<2&&(tagCharge!=probetkCharge)&&Min$(abs(eleD0[imatchele])-abs(probetkd0))>0");
   trmc->Project("hden_mc","phoSCEt_notag","phoSCEt_notag>2&&probetkPt<2&&(tagCharge!=probetkCharge)");
   ...
   TLegend *tleg = new TLegend(0.6,0.2,0.9,0.4);
   tleg->SetBorderSize(0);
   tleg->AddEntry(gtkdata,Form("Data: %.3f^{+%.3f}_{%.3f}",efftkdata,efftkdataup,efftkdatadown),"LP");
   tleg->AddEntry(gtkmc,Form("MC: %.3f^{+%.3f}_{%.3f}",efftkmc,efftkmcup,efftkmcdown),"LP");

   TCanvas *c1 = new TCanvas();
   haxes_tk->Draw();
   haxes_tk->GetYaxis()->SetRangeUser(0.5,1.1);
   c1->RedrawAxis();
   gtkdata->Draw("P");
   gtkmc->Draw("P");
   tleg->Draw();
   ...
   tleg2->AddEntry(geldata,Form("Data: %.3f^{+%.3f}_{%.3f}",effeldata,effeldataup,effeldatadown),"LP");
   tleg2->AddEntry(gelmc,Form("MC: %.3f^{+%.3f}_{%.3f}",effelmc,effelmcup,effelmcdown),"LP");

   haxes_el->Draw();
   haxes_el->GetYaxis()->SetRangeUser(0,0.5);
   c1->RedrawAxis();
   geldata->Draw("P");
   gelmc->Draw("P");
   tleg2->Draw();
