...
void scged_efficiency_diphoton( std::string infile_Forest = "filelist.txt",
      std::string out = "scged_photon_efficiency_gen_reco_match_v2.root") 
{
   ...
   treeHLT->SetBranchStatus("NL1IsolEm",1);
   treeHLT->SetBranchStatus("L1IsolEm*",1);
   treeHLT->SetBranchStatus("NL1NIsolEm",1);
   treeHLT->SetBranchStatus("L1NIsolEm*",1);
   treeHLT->SetBranchAddress("NL1IsolEm", &NL1IsolEm, &b_NL1IsolEm);
   treeHLT->SetBranchAddress("L1IsolEmEt", L1IsolEmEt, &b_L1IsolEmEt);
   treeHLT->SetBranchAddress("L1IsolEmE", L1IsolEmE, &b_L1IsolEmE);
   treeHLT->SetBranchAddress("L1IsolEmEta", L1IsolEmEta, &b_L1IsolEmEta);
   treeHLT->SetBranchAddress("L1IsolEmPhi", L1IsolEmPhi, &b_L1IsolEmPhi);
   treeHLT->SetBranchAddress("NL1NIsolEm", &NL1NIsolEm, &b_NL1NIsolEm);
   treeHLT->SetBranchAddress("L1NIsolEmEt", L1NIsolEmEt, &b_L1NIsolEmEt);
   treeHLT->SetBranchAddress("L1NIsolEmE", L1NIsolEmE, &b_L1NIsolEmE);
   treeHLT->SetBranchAddress("L1NIsolEmEta", L1NIsolEmEta, &b_L1NIsolEmEta);
   treeHLT->SetBranchAddress("L1NIsolEmPhi", L1NIsolEmPhi, &b_L1NIsolEmPhi);
                     ...
                     for(int bb = aa+1 ; bb <nMC; ++bb ){    
                        ...
                        if(fabs(mcEta->at(aa)) < 2.4 && mcEt->at(aa) > 2.0 && fabs(mcEta->at(bb)) < 2.4 && mcEt->at(bb) > 2.0 ){ 
                           ...
                           if (getDR(phoSCEta->at(i), phoSCPhi->at(i), mcEta->at(aa), mcPhi->at(aa)) < 0.3 || 
                              ...
                              TLorentzVector pho0, pho1;
                              pho0.SetPtEtaPhiM(phoSCEt->at(i),phoSCEta->at(i),phoSCPhi->at(i),0);
                              pho1.SetPtEtaPhiM(phoSCEt->at(j),phoSCEta->at(j),phoSCPhi->at(j),0);
                              // L1-offline matching
                              for (int i=0; i<NL1IsolEm; i++) {
                                 if (L1IsolEmEt[i]<=0) continue;
                                 TLorentzVector tlv; tlv.SetPtEtaPhiM(L1IsolEmEt[i],L1IsolEmEta[i],L1IsolEmPhi[i],0);
                                 double dr0 = tlv.DeltaR(pho0);
                                 double dr1 = tlv.DeltaR(pho1);
                                 if (dr0<minDR0) {
                                    minDR0 = dr0;
                                    EtMinDR0 = L1IsolEmEt[i];
                                 }
                                 if (dr1<minDR1) {
                                    minDR1 = dr1;
                                    EtMinDR1 = L1IsolEmEt[i];
                                 }
                              }
                              for (int i=0; i<NL1NIsolEm; i++) {
                                 if (L1NIsolEmEt[i]<=0) continue;
                                 TLorentzVector tlv; tlv.SetPtEtaPhiM(L1NIsolEmEt[i],L1NIsolEmEta[i],L1NIsolEmPhi[i],0);
                                 double dr0 = tlv.DeltaR(pho0);
                                 double dr1 = tlv.DeltaR(pho1);
                                 if (dr0<minDR0) {
                                    minDR0 = dr0;
                                    EtMinDR0 = L1NIsolEmEt[i];
                                 }
                                 if (dr1<minDR1) {
                                    minDR1 = dr1;
                                    EtMinDR1 = L1NIsolEmEt[i];
                                 }
                              }


                              // fill histos
                              bool pho0okden=false, pho1okden=false, pho0oknum=false, pho1oknum=false;
                              pho0okden = (pho_swissCrx->at(i)<0.95 && fabs(pho_seedTime->at(i))<3.6 && fabs(pho0.Eta()<2.4));
                              pho1okden = (pho_swissCrx->at(j)<0.95 && fabs(pho_seedTime->at(j))<3.6 && fabs(pho1.Eta()<2.4));
                              pho0oknum = pho0okden && minDR0<DRmax && EtMinDR0>=2;
                              pho1oknum = pho1okden && minDR1<DRmax && EtMinDR1>=2;

                              TLorentzVector dipho = pho0+pho1;

                              if (pho0okden && dipho.M()>5 && dipho.Pt()<1) {
                                 double pt0 = pho0.Pt();
                                 hden->Fill(pt0);
                                 if (fabs(pho0.Eta())<1.5) hden_EB->Fill(pt0);
                                 else hden_EE->Fill(pt0);
                                 if (pho0oknum) {
                                    hnum->Fill(pt0);
                                    if (fabs(pho0.Eta())<1.5) hnum_EB->Fill(pt0);
                                    else hnum_EE->Fill(pt0);
                                 }
                              }
                              if (pho1okden && dipho.M()>5 && dipho.Pt()<1) {
                                 double pt1 = pho1.Pt();
                                 hden->Fill(pt1);
                                 if (fabs(pho1.Eta())<1.5) hden_EB->Fill(pt1);
                                 else hden_EE->Fill(pt1);
                                 if (pho1oknum) {
                                    hnum->Fill(pt1);
                                    if (fabs(pho1.Eta())<1.5) hnum_EB->Fill(pt1);
                                    else hnum_EE->Fill(pt1);
                                 }
                              }

                              if (pho0okden && pho1okden && dipho.M()>5 && dipho.Pt()<1) {
                                 nden++;
                                 if(HLT_HIUPCL1DoubleEG2NotHF2_v1==1) nnum++;
                              }
